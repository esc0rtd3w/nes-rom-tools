...
int basePalette[64]={//from a picture found in internet
	0x7f7f7f,0xff0000,0xbf0000,0xbf2b47,
	0x870097,0x2300ab,0x0013ab,0x00178b,
	0x003053,0x007800,0x006b00,0x005b00,
	0x584300,0x000000,0x000000,0x000000,
	0xbfbfbf,0xf87800,0xf85800,0xff476b,
	0xcd00db,0x5b00e7,0x0038f8,0x135fe7,
	0x007faf,0x00b800,0x00ab00,0x47ab00,
	0x8b8b00,0x000000,0x000000,0x000000,
	0xf8f8f8,0xffbf3f,0xff886b,0xf87898,
	0xf878f8,0x9858f8,0x5878f8,0x47a3ff,
	0x00b8f8,0x18f8b8,0x57db5b,0x98f858,
	0xdbeb00,0x787878,0x000000,0x000000,
	0xffffff,0xffe7a7,0xf8b8b8,0xf8b8d8,
	0xf8b8f8,0xc3a7fb,0xb0d0f0,0xabe3ff,
	0x7bdbfb,0x78f8d8,0xb8f8b8,0xd8f8b8,
	0xffff00,0xf8d8f8,0x000000,0x000000
};
...
void palette_calc(void)
{
	...
	if(!(ppuMask&0x01))
	{
		for(i=0;i<64;i++)
		{
			...
			if(g>1.0) g=1.0;
			if(b>1.0) b=1.0;
			palette[i]=(((int)(255.0*r))<<16)|(((int)(255.0*g))<<8)|((int)(255.0*b));
		}
	}
	else
	{
...
void __fastcall TFormMain::DrawCol(int x,int y,int c,bool sel)
{
	...
	r.right=x+20;
	r.Bottom=y+20;
	PaintBoxPal->Canvas->Brush->Color=TColor(palette[c]);
...
void __fastcall TFormMain::DrawPalettes(void)
{
	...
	for(i=0;i<4;i++)
	{
		...
		for(j=0;j<14;j++)
		{
			PaintBoxPal->Canvas->Brush->Color=TColor(palette[pp]);
			r.left=x;
			r.top=y;
			r.right=x+16;
			r.bottom=y+16;
...
void __fastcall TFormMain::CopyMapCodeC(void)
{
	...
	if(nameSelection.left>=0&&nameSelection.top>=0)
	{
		...
		for(i=0;i<h;i++)
		{
			...
			for(j=0;j<w;j++)
			{
				sprintf(buf,"0x%2.2x%c",nameTable[(y+i)*32+x+j],j<w-1?',':'\n');
				strcat(str,buf);
			}
		}
...
void __fastcall TFormMain::UpdateStats(void)
{
	...
	if(tileXC>=0&&tileYC>=0)
	{
		...
		LabelStats->Caption="Tile:$"+IntToHex(tile,2)+" ("+IntToStr(cnt)+" entries)";
	}
...
void __fastcall TFormMain::MSaveNameTableOnlyClick(TObject *Sender)
{
	...
	if(SaveDialogName->Execute())
	{
		...
		if(FileExists(name))
		{
			if(Application->MessageBox(("File "+name+" already exists, overwrite?").c_str(),"Confirm",MB_YESNO)!=IDYES) return;
		}
	...
	if(OpenDialogImport->Execute())
	{
		...
		if(ext=="bmp")//import BMP file
		{
			...
			if(file)
			{
				...
				if(cnt>=256) Application->MessageBox(("Too many unique characters ("+IntToStr(cnt)+")").c_str(),"Warning",MB_OK);
